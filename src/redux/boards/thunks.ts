...
export const getBoardsThunk = thunkMiddleware<
  Board[],
  void,
  { rejectValue: { message: string } }
>('boards/getBoard', async () => {
  return await getAllBoards();
});

export const addBoardThunk = thunkMiddleware<
  Board,
  Board,
  { rejectValue: { message: string } }
>('boards/addBoard', async (body) => {
  return await addBoard(body);
});

export const getBoardById = thunkMiddleware<
  Board,
  string,
  { rejectValue: { message: string } }
>('boards/singleBoard', async (id) => {
